...
bool CCalibration::readPatternImg()
{
    ...
    do
    {
        stringstream ss;
        ss<<imgNum;
        string path=patternImgPath+ss.str()+"_R.jpg";
        img=imread(path, 0);
        if (!img.data)
            break;
...
void CCalibration::calibProcess()
{
    ...
    int successImgNum=0;
    int count=0;
    cout<<"********start to find chess board corners!********"<<endl;
    Mat image,scaleImg;
...
Mat CCalibration::get_camK()
{
	return camK;
}

Mat CCalibration::get_camDiscoeff()
{
	return camDiscoeff;
}
